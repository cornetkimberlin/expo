...
export default function DOMComponent(props: import('expo/dom').DOMProps) {
  return (
    <div>
      <h1>Hello, world!</h1>
    </div>
...
You may want to measure the size of a DOM component and report it back to the native side (for example, native scrolling). This can be done using an `autoSize` prop or a manual native action:

### The `autoSize` prop

You can use the `autoSize` prop to automatically measure the size of a DOM component and resize the native view coresponsingly:

```tsx App.tsx (native)
import DOMComponent from './my-component';

export default function Route() {
  return <MyComponent autoSize={true} />;
}
```
